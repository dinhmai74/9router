...
import { consumeCursorSdkAgentEvents } from "../services/cursorAgentSdk.js";
import { buildCursorSdkAuthFailureResponse, resolveCursorSdkApiKey } from "../services/cursorSdkAuth.js";
import zlib from "zlib";

const CURSOR_SDK_AGENT_URL = "cursor-sdk://agent";
...
export class CursorExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials) {
    ...
    if (!accessToken) {
      throw new Error("Access token or API key is required for Cursor API");
    }
  ...
  async executeAgent({ model, body, stream, credentials, signal }) {
    const authFailure = buildCursorSdkAuthFailureResponse({
      credentials,
      body,
      url: CURSOR_SDK_AGENT_URL,
    });
    if (authFailure) return authFailure;

    const url = CURSOR_SDK_AGENT_URL;
    ...
    const consume = (onEvent) => consumeCursorSdkAgentEvents({
      model,
      ...
      start(controller) {
        ...
        consume((event) => {
          ...
          if (event.type === "text") {
            ...
            finish([sseChunk({ error: { message: event.value, type: "api_error" } }), SSE_DONE]);
          } else if (event.type === "done") {
            finish([chatChunkSse({ id: responseId, created, model, delta: {}, finishReason: "stop" }), SSE_DONE]);
          }
        }).catch((error) => {
          if (settled) return;
          settled = true;
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    ...
    if (apiKey || isAgentTextRequest(body)) {
      try {
